...
def jumpRight(prog: String, pc: Int, level: Int) : Int = {
  val temp = prog.toList
  if (temp(pc).equals('[')) jumpRight(prog, pc+1, level+1)
  else if (temp(pc).equals(']')) jumpRight(prog,pc+1,level-1)
  else  {
    if (level == -1) {pc}
    else if (pc == prog.size-1) {pc+1} 
...
def jumpLeft(prog: String, pc: Int, level: Int) : Int = {
  val temp = prog.toList
  if (temp(pc).equals(']')) jumpLeft(prog, pc-1,level+1)
  else if (temp(pc).equals('[')) {
    if (level-1 == -1) {pc+1}
    else if (pc == 0) {pc-1}
